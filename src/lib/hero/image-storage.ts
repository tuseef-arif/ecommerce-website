/**
 * Hero slide image storage:
 * - Persists uploads to Vercel Blob when `BLOB_READ_WRITE_TOKEN` is set.
 * - Otherwise writes to `public/uploads/hero/` on local disk (dev / self-host).
 * - On Vercel runtime without a blob token, refuses to write (no persistent disk).
 *
 * Stores the public URL/path string on `HeroSlide.imagePath`. The same string is
 * later passed to `deleteHeroImageIfOwned` to clean up safely on replace/delete.
 *
...
const HERO_UPLOAD_DIR = path.join("public", "uploads", "hero");
const HERO_UPLOAD_WEB_PREFIX = "/uploads/hero";
const HERO_BLOB_PREFIX = "hero";
...
const resolveSafeHeroImageDiskPath = (webPath: string): string | null => {
  if (!webPath.startsWith(`${HERO_UPLOAD_WEB_PREFIX}/`)) return null;
  const relativeFromPublic = webPath.replace(/^\//, "");
  const abs = path.resolve(process.cwd(), "public", relativeFromPublic);
  const root = path.resolve(process.cwd(), HERO_UPLOAD_DIR);
  if (abs !== root && !abs.startsWith(`${root}${path.sep}`)) return null;
  return abs;
};

export type SaveHeroImageError =
  | "invalid_format"
  | "too_large"
  | "blob_required"
  | "upload_failed";

export type SaveHeroImageResult =
  | { ok: true; webPath: string }
  | { ok: false; error: SaveHeroImageError };

/**
 * Saves an uploaded hero image and returns its public URL (`webPath`).
 * Magic-byte validates the buffer; never trusts the supplied filename or MIME.
 */
export const saveHeroImage = async (
  file: File,
): Promise<SaveHeroImageResult> => {
  if (file.size === 0) {
  ...
  if (blobToken) {
    try {
      const blob = await put(`${HERO_BLOB_PREFIX}/${fileName}`, buf, {
        access: "public",
        token: blobToken,
      ...
      console.error("hero-image blob upload failed", { error });
  ...
  const dirAbs = path.join(process.cwd(), HERO_UPLOAD_DIR);
  ...
  try {
    ...
    console.error("hero-image local write failed", { diskPath, error });
    return { ok: false, error: "upload_failed" };
  }

  return { ok: true, webPath: `${HERO_UPLOAD_WEB_PREFIX}/${fileName}` };
};

/**
 * Removes a previously stored hero image (best-effort).
 * Only deletes paths/URLs we know we own (our blob host or our upload dir).
 */
export const deleteHeroImageIfOwned = async (
  ...
  const abs = resolveSafeHeroImageDiskPath(webPath);
  if (!abs) return;
  try {
    await unlink(abs);
  } catch {
